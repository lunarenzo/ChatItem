package io.github.lunatech.chatitem.messaging.adapter.task;
...
public class BukkitTaskAdapter implements TaskAdapter {
    ...
    public void init(Runnable runnable, long delay, long interval, TimeUnit timeUnit) {
        Bukkit.getAsyncScheduler().runAtFixedRate(AbstractChatItem.getInstance(), (task) -> runnable.run(), delay, interval, timeUnit);
    }

    @Override
    public void cancel() {
        Bukkit.getAsyncScheduler().cancelTasks(AbstractChatItem.getInstance());
    }
}
